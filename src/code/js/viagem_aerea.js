...
const dadosIniciais = {
    viagem: [
      { "id": generateUUID(), "estado_ida": "MG", "aeroporto_ida": "CSS", "estado_destino": "SP", "aeroporto_destino": "CDD", "num_passageiros": "1", "id_usuario": "2"},
    ]
  };
  
  // Inicializa o usuarioCorrente e banco de dados de usuários da aplicação de Login
function initViagemAero() {
    ...
    var membrosJSON = localStorage.getItem('db_viagensaero');
    ...
    if (!membrosJSON) {  // Se NÃO há dados no localStorage
      ...
      db_viagensaero = dadosIniciais;
  
      // Salva os dados iniciais no local Storage convertendo-os para string antes
      localStorage.setItem('db_viagensaero', JSON.stringify(dadosIniciais));
    }
    else {  // Se há dados no localStorage
  
      // Converte a string JSON em objeto colocando no banco de dados baseado em JSON
      db_viagensaero = JSON.parse(membrosJSON);
    }
  };
  ...
  function addViagem(estado, cidade, estado_destino, cidade_destino, num_passageiros) {
    ...
    db_viagensaero.viagem.push(viagem);
  
    // Salva o novo banco de dados com o novo usuário no localStorage
    localStorage.setItem('db_viagensaero', JSON.stringify(db_viagensaero));
